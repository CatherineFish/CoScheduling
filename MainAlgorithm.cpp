...
void MainAlgorithm:: MainLoop(System* CurSystem, int Depth, int Search)
{
    int repeat = 0;
    while (Unplanned.size())
    {
        ...
        if (CheckResult == 1)
        {
            ...
            LimitedSearch CurLimitedSerch(Mood, Depth, Search, true);
        ...
        } else if (CheckResult == 2)
        {
            ...
            LimitedSearch CurLimitedSerch(Mood, Depth, Search);
            CurPC = CurLimitedSerch.MainLoop(0, QueueForPlan[0], Planned, CurSystem, Unplanned); 
        }
        for (const auto & JustJob: CurSystem->SystemJob)
        {
            JustJob->Tried.clear();
        }
...
void MainAlgorithm:: PrintJobSystem(System* CurSystem)
{
    ...
    {
        ////std::cout << std::endl;
        ////std::cout << "     Job " << std::find(CurSystem->SystemJob.begin(), CurSystem->SystemJob.end(), Unplanned[i]) - CurSystem->SystemJob.begin()  << std::endl;
